...
class MeningiomaSegmentationTask(BaseTask):
    ...
    def __init__(self):
        ...
        self.nsd_metric = SurfaceDistanceMetric(
            include_background=False, symmetric=True, distance_metric="euclidean"
        )
    ...
    def evaluate(
        ...
        for pred_path, subject_id in prediction_files:
            if subject_id in available_subjects:
                ...
                if metrics:
                    ...
                    valid_evaluations += 1
                    self.logger.info(
                        f"✅ {subject_id}: DSC={metrics['dice']:.4f}, NSD={metrics['nsd']:.4f}"
                    )
                else:
        ...
        self.logger.info(
            f"Final metrics - DSC: {final_metrics['dice']:.4f}, "
    ...
    def _find_prediction_files(
        self, output_path: Path, task_output_dir: Path
    ) -> List[Tuple[Path, str]]:
    ...
    def _evaluate_single_subject(
        self, pred_path: Path, subject_id: str
    ...
    def _save_subject_metrics_csv(
        self, subject_metrics: List[Dict], task_output_dir: Path
    ) -> None:
    ...
    def _compute_average_metrics(self, subject_metrics: List[Dict]) -> Dict[str, Any]:
        ...
        return {"dice": float(np.mean(dice_scores)), "nsd": float(np.mean(nsd_scores))}
