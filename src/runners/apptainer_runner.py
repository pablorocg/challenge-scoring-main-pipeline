...
class ApptainerRunner:
    """Container runner for medical imaging inference."""

    def __init__(self):
        self.logger = get_logger(__name__)

    def run_inference(
        self, container_path: Path, task, output_path: Path, task_output_dir: Path
    ) -> bool:
        ...
        for subject_dir in subject_dirs:
            output_file = self._process_subject(
        ...
        if task.name == "Meningioma Segmentation":
            ...
            self.logger.info(
                f"Completed segmentation for {len(subject_outputs)} subjects"
            )
            return True
        else:
    ...
    def _process_subject(
        self, container_path: Path, subject_dir: Path, task, task_output_dir: Path
    ) -> Optional[Path]:
        ...
        output_file = (
            task_output_dir / f"{subject_dir.name}_output{task.output_extension}"
        )
        instance_name = (
            f"fomo_{container_path.stem}_{subject_dir.name}_{int(time.time())}"
        )

        with self._container_instance(
            container_path, subject_dir, instance_name, task_output_dir
        ) as instance:
    ...
    def _get_modalities(
        self, subject_dir: Path, task_name: str
    ) -> List[Tuple[Path, str]]:
    ...
    def _start_instance(
        ...
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=SETTINGS.INSTANCE_START_TIMEOUT
        )
    ...
    def _stop_instance(self, instance_name: str):
        ...
        subprocess.run(
            [SETTINGS.APPTAINER_EXECUTABLE, "instance", "stop", instance_name],
            capture_output=True,
            timeout=SETTINGS.INSTANCE_STOP_TIMEOUT,
        )

    def _run_inference(
        self, instance_name: str, modalities: List[Tuple[Path, str]], output_file: Path
    ) -> bool:
        ...
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=SETTINGS.CONTAINER_TIMEOUT
        )

        success = result.returncode == 0 and output_file.exists()
        if not success:
            self.logger.error(
                f"Inference failed: {result.stderr or 'Output not created'}"
            )
