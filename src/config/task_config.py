...
"""Task-specific configuration for FOMO evaluation system."""
...
def validate_task_modalities(task_name: str, available_modalities: List[str]) -> Dict[str, Any]:
    """Validate if available modalities meet task requirements."""
    config = get_task_config(task_name)
    
    missing_required = [mod for mod in config.required if mod not in available_modalities]
    has_optional = any(mod in available_modalities for mod in config.optional)
    
    is_valid = (
        len(missing_required) == 0 and 
        (not config.optional or has_optional)
    )
    ...
    return {
        ...
        "available_optional": [mod for mod in config.optional if mod in available_modalities]
    }